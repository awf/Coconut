...
array_number_t linalg_rodrigues_rotate_point(array_number_t rot, array_number_t x) {
	...
	if((sqtheta) != (0)) {
		...
		ite27 = linalg_add_vec(linalg_add_vec(v1, v2), linalg_mult_by_scalar(w, tmp));
	} else {
		
		ite27 = linalg_add_vec(x, linalg_cross(rot, x));
	}
	return ite27;
}
...
env_t_31* make_env_t_31() {
	env_t_31* env = (env_t_31*)malloc(sizeof(env_t_31));
	
	return env;
}

value_t lambda31(env_t_31* env28, number_t w) {
...
array_number_t linalg_w_err(array_number_t w) {
	closure_t* closure30 = make_closure(lambda31, make_env_t_31());
	return array_map(closure30, w);
}
...
env_t_40* make_env_t_40(array_array_number_t x,array_number_t w,array_array_number_t obs,array_array_number_t feat,array_array_number_t cams) {
	env_t_40* env = (env_t_40*)malloc(sizeof(env_t_40));
	env->x = x;
	env->w = w;
	env->obs = obs;
	env->feat = feat;
...
value_t lambda40(env_t_40* env37, number_t i) {
	array_array_number_t x36 = env37->x;
	array_number_t w35 = env37->w;
	array_array_number_t obs34 = env37->obs;
	array_array_number_t feat33 = env37->feat;
	array_array_number_t cams32 = env37->cams;
	value_t res;
	res.array_number_t_value = linalg_compute_reproj_err(cams32->arr[(int)(obs34->arr[(int)(i)]->arr[0])], x36->arr[(int)(obs34->arr[(int)(i)]->arr[1])], w35->arr[(int)(i)], feat33->arr[(int)(i)]);
	return res;
}
array_array_number_t linalg_reproj_err(array_array_number_t cams, array_array_number_t x, array_number_t w, array_array_number_t obs, array_array_number_t feat) {
	...
	closure_t* closure39 = make_closure(lambda40, make_env_t_40(x,w,obs,feat,cams));
	return array_map_to_matrix(closure39, range);
}
...
env_t_64* make_env_t_64(array_number_t one_cam) {
	env_t_64* env = (env_t_64*)malloc(sizeof(env_t_64));
	env->one_cam = one_cam;
	return env;
}

value_t lambda64(env_t_64* env42, number_t x) {
	array_number_t one_cam41 = env42->one_cam;
	value_t res;
	res.array_number_t_value = one_cam41;
	return res;
}
typedef struct env_t_65 {
	array_number_t one_x;
} env_t_65;
env_t_65* make_env_t_65(array_number_t one_x) {
	env_t_65* env = (env_t_65*)malloc(sizeof(env_t_65));
	env->one_x = one_x;
	return env;
}

value_t lambda65(env_t_65* env46, number_t x) {
	array_number_t one_x45 = env46->one_x;
	value_t res;
	res.array_number_t_value = one_x45;
	return res;
}
typedef struct env_t_66 {
	number_t one_w;
} env_t_66;
env_t_66* make_env_t_66(number_t one_w) {
	env_t_66* env = (env_t_66*)malloc(sizeof(env_t_66));
	env->one_w = one_w;
	return env;
}

value_t lambda66(env_t_66* env50, number_t x) {
	number_t one_w49 = env50->one_w;
	value_t res;
	res.number_t_value = one_w49;
	return res;
}
typedef struct env_t_67 {
	array_number_t one_feat;
} env_t_67;
env_t_67* make_env_t_67(array_number_t one_feat) {
	env_t_67* env = (env_t_67*)malloc(sizeof(env_t_67));
	env->one_feat = one_feat;
	return env;
}

value_t lambda67(env_t_67* env54, number_t x) {
	array_number_t one_feat53 = env54->one_feat;
	value_t res;
	res.array_number_t_value = one_feat53;
	return res;
}
...
env_t_68* make_env_t_68(index_t n,index_t m) {
...
value_t lambda68(env_t_68* env59, number_t x) {
	index_t n58 = env59->n;
	index_t m57 = env59->m;
	array_number_t array62 = (array_number_t)malloc(sizeof(int) * 2);
	array62->length=2;
	array62->arr = (number_t*)malloc(sizeof(number_t) * 2);
	array62->arr[0] = (double)(((int)(x)) % (n58));
	array62->arr[1] = (double)(((int)(x)) % (m57));;
	value_t res;
	res.array_number_t_value = array62;
	return res;
}
array_array_number_t linalg_run_ba_from_file(string_t fn) {
	...
	closure_t* closure44 = make_closure(lambda64, make_env_t_64(one_cam));
	array_array_number_t cam = array_map_to_matrix(closure44, array_range(1, n));
	array_number_t one_x = linalg_vectorRead(fn, 2);
	closure_t* closure48 = make_closure(lambda65, make_env_t_65(one_x));
	array_array_number_t x = array_map_to_matrix(closure48, array_range(1, m));
	number_t one_w = linalg_numberRead(fn, 3);
	closure_t* closure52 = make_closure(lambda66, make_env_t_66(one_w));
	array_number_t w = array_map(closure52, array_range(1, p));
	array_number_t one_feat = linalg_vectorRead(fn, 4);
	closure_t* closure56 = make_closure(lambda67, make_env_t_67(one_feat));
	array_array_number_t feat = array_map_to_matrix(closure56, array_range(1, p));
	closure_t* closure61 = make_closure(lambda68, make_env_t_68(n,m));
	array_array_number_t obs = array_map_to_matrix(closure61, array_range(0, (p) - (1)));
	timer_t t = tic();
...
env_t_73* make_env_t_73(number_t mx) {
	env_t_73* env = (env_t_73*)malloc(sizeof(env_t_73));
	env->mx = mx;
	return env;
}

value_t lambda73(env_t_73* env70, number_t x) {
	number_t mx69 = env70->mx;
	value_t res;
	res.number_t_value = exp((x) - (mx69));
	return res;
}
number_t linalg_logsumexp(array_number_t arr) {
	number_t mx = array_max(arr);
	closure_t* closure72 = make_closure(lambda73, make_env_t_73(mx));
	number_t semx = array_sum(array_map(closure72, arr));
	return (log(semx)) + (mx);
}
typedef struct env_t_78 {
	number_t a;
} env_t_78;
env_t_78* make_env_t_78(number_t a) {
	env_t_78* env = (env_t_78*)malloc(sizeof(env_t_78));
	env->a = a;
	return env;
}

value_t lambda78(env_t_78* env75, number_t j) {
	number_t a74 = env75->a;
	value_t res;
	res.number_t_value = gamma_ln((a74) + ((0.5) * ((1) - ((double)(j)))));
	return res;
}
number_t linalg_log_gamma_distrib(number_t a, number_t p) {
	closure_t* closure77 = make_closure(lambda78, make_env_t_78(a));
	return (log(pow(3.14159265358979, (0.25) * ((p) * ((p) - (1)))))) + (array_sum(array_map(closure77, array_range(1, (int)(p)))));
}
...
array_array_number_t linalg_to_pose_params(array_number_t theta, index_t n_bones) {
	...
	finger5->arr[0] = array96;
	finger5->arr[1] = array97;
	finger5->arr[2] = array98;
	finger5->arr[3] = array99;;
...
array_array_number_t linalg_euler_angles_to_rotation_matrix(array_number_t xzy) {
	...
	array103->arr[0] = cos(ty);
	array103->arr[1] = 0;
	array103->arr[2] = sin(ty);;
	...
	array105->arr[0] = -(sin(ty));
	array105->arr[1] = 0;
	array105->arr[2] = cos(ty);;
...
array_array_number_t linalg_make_relative(array_number_t pose_params, array_array_number_t base_relative) {
	...
	array110->length=1;
	array110->arr = (number_t*)malloc(sizeof(number_t) * 1);
	array110->arr[0] = 0;;
	array_number_t array111 = (array_number_t)malloc(sizeof(int) * 2);
	array111->length=1;
	array111->arr = (number_t*)malloc(sizeof(number_t) * 1);
	array111->arr[0] = 0;;
	array_number_t array112 = (array_number_t)malloc(sizeof(int) * 2);
	array112->length=1;
	array112->arr = (number_t*)malloc(sizeof(number_t) * 1);
	array112->arr[0] = 0;;
	array_array_number_t array109 = (array_array_number_t)malloc(sizeof(int) * 2);
	array109->length=3;
	array109->arr = (array_number_t*)malloc(sizeof(array_number_t) * 3);
	array109->arr[0] = array110;
	array109->arr[1] = array111;
	array109->arr[2] = array112;;
	array_number_t array114 = (array_number_t)malloc(sizeof(int) * 2);
	array114->length=4;
	array114->arr = (number_t*)malloc(sizeof(number_t) * 4);
	array114->arr[0] = 0;
	array114->arr[1] = 0;
	array114->arr[2] = 0;
	array114->arr[3] = 1;;
	array_array_number_t array113 = (array_array_number_t)malloc(sizeof(int) * 2);
	array113->length=1;
	array113->arr = (array_number_t*)malloc(sizeof(array_number_t) * 1);
	array113->arr[0] = array114;;
	array_array_number_t T = matrix_concat(linalg_matrixConcatCol(R, array109), array113);
...
env_t_121* make_env_t_121(array_array_number_t pose_params,index_t offset,array_array_array_number_t base_relatives) {
	env_t_121* env = (env_t_121*)malloc(sizeof(env_t_121));
...
value_t lambda121(env_t_121* env118, number_t i_bone) {
	array_array_number_t pose_params117 = env118->pose_params;
	index_t offset116 = env118->offset;
	array_array_array_number_t base_relatives115 = env118->base_relatives;
	value_t res;
	res.array_array_number_t_value = linalg_make_relative(pose_params117->arr[((int)(i_bone)) + (offset116)], base_relatives115->arr[(int)(i_bone)]);
	return res;
}
array_array_array_number_t linalg_get_posed_relatives(index_t n_bones, array_array_number_t pose_params, array_array_array_number_t base_relatives) {
	index_t offset = 3;
	closure_t* closure120 = make_closure(lambda121, make_env_t_121(pose_params,offset,base_relatives));
	return array_map_to_matrix3d(closure120, array_range(0, (n_bones) - (1)));
}
...
array_array_number_t linalg_angle_axis_to_rotation_matrix(array_number_t angle_axis) {
	...
	array_array_number_t ite122 = NULL;
	if((n) < (0.0001)) {
	...
	array123->length=3;
	array123->arr = (array_number_t*)malloc(sizeof(array_number_t) * 3);
	array123->arr[0] = array124;
	array123->arr[1] = array125;
	array123->arr[2] = array126;;
		ite122 = array123;
	} else {
	...
	array128->arr[0] = ((x) * (x)) + (((1) - ((x) * (x))) * (c));
	array128->arr[1] = (((x) * (y)) * ((1) - (c))) - ((z) * (s));
	array128->arr[2] = (((x) * (z)) * ((1) - (c))) + ((y) * (s));;
		array_number_t array129 = (array_number_t)malloc(sizeof(int) * 2);
	array129->length=3;
	array129->arr = (number_t*)malloc(sizeof(number_t) * 3);
	array129->arr[0] = (((x) * (y)) * ((1) - (c))) + ((z) * (s));
	array129->arr[1] = ((y) * (y)) + (((1) - ((y) * (y))) * (c));
	array129->arr[2] = (((y) * (z)) * ((1) - (c))) - ((x) * (s));;
		array_number_t array130 = (array_number_t)malloc(sizeof(int) * 2);
	array130->length=3;
	array130->arr = (number_t*)malloc(sizeof(number_t) * 3);
	array130->arr[0] = (((x) * (z)) * ((1) - (c))) - ((y) * (s));
	array130->arr[1] = (((z) * (y)) * ((1) - (c))) + ((x) * (s));
	array130->arr[2] = ((z) * (z)) + (((1) - ((z) * (z))) * (c));;
		array_array_number_t array127 = (array_array_number_t)malloc(sizeof(int) * 2);
	array127->length=3;
	array127->arr = (array_number_t*)malloc(sizeof(array_number_t) * 3);
	array127->arr[0] = array128;
	array127->arr[1] = array129;
	array127->arr[2] = array130;;
		ite122 = array127;
	}
	return ite122;
}
...
env_t_140* make_env_t_140(array_number_t scale) {
	env_t_140* env = (env_t_140*)malloc(sizeof(env_t_140));
	env->scale = scale;
	return env;
}

value_t lambda140(env_t_140* env132, array_number_t row) {
	array_number_t scale131 = env132->scale;
	value_t res;
	res.array_number_t_value = linalg_mult_vec_elementwise(row, scale131);
	return res;
}
typedef struct env_t_141 {
	value_t dummy_variable;
} env_t_141;
env_t_141* make_env_t_141() {
	env_t_141* env = (env_t_141*)malloc(sizeof(env_t_141));
	
	return env;
}

value_t lambda141(env_t_141* env135, number_t x) {
...
array_array_number_t linalg_apply_global_transform(array_array_number_t pose_params, array_array_number_t positions) {
	...
	closure_t* closure134 = make_closure(lambda140, make_env_t_140(scale));
	array_array_number_t R1 = matrix_map(closure134, R);
	array_array_number_t array138 = (array_array_number_t)malloc(sizeof(int) * 2);
	array138->length=1;
	array138->arr = (array_number_t*)malloc(sizeof(array_number_t) * 1);
	array138->arr[0] = pose_params->arr[2];;
	array_array_number_t T = linalg_matrixConcatCol(R1, matrix_transpose(array138));
	closure_t* closure137 = make_closure(lambda141, make_env_t_141());
	array_number_t ones = array_map(closure137, array_range(1, positions->arr[0]->length));
	array_array_number_t array139 = (array_array_number_t)malloc(sizeof(int) * 2);
	array139->length=1;
	array139->arr = (array_number_t*)malloc(sizeof(array_number_t) * 1);
	array139->arr[0] = ones;;
	array_array_number_t positions_homog = matrix_concat(positions, array139);
	return matrix_mult(T, positions_homog);
}
typedef struct env_t_167 {
	value_t dummy_variable;
} env_t_167;
env_t_167* make_env_t_167() {
	env_t_167* env = (env_t_167*)malloc(sizeof(env_t_167));
	
	return env;
}

value_t lambda167(env_t_167* env142, number_t r) {
...
void linalg_test1(array_number_t dum) {
	...
	matrix_print(p);
	closure_t* closure144 = make_closure(lambda167, make_env_t_167());
	array_array_number_t base_rel = array_map_to_matrix(closure144, array_range(1, 4));
