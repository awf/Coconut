...
let hand_objective_complicated (param:_[]) (us:_[]) (data:HandData) =
  let pose_params = to_pose_params param data.model.bone_names.Length
  
  let vertex_positions = get_skinned_vertex_positions data.model pose_params
  
  let get_hand_point (u:_[]) (verts:int[]) = 
    ...
    let us = 
      if read_us then
        let tmp = [|for line in instance.[offset..offset+n_pts-1] do
                        for elem in line do
                          yield Double.Parse elem|]
        offset <- offset + n_pts
        tmp
      else
        [||]
      
    let theta = [|for i=0 to n_params-1 do yield Double.Parse instance.[offset+i].[0]|]
