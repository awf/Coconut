...
int main(int argc, char** argv)
{
	...
	char* file = argv[1];
	array_number_t nmp = TOP_LEVEL_linalg_vectorRead_dps(empty_storage, file, 0, 3, 0, 0, 3);
	card_t n = (int)(nmp->arr[0]);
	card_t m = (int)(nmp->arr[1]);
	card_t p = (int)(nmp->arr[2]);
	TOP_LEVEL_usecases_ba_run_ba_from_file_dps(empty_storage, file, n, m, p, 0, n, m, p);
