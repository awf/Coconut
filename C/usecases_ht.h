...
typedef struct env_t_198 {
	array_array_array_number_t relatives;
	array_number_t parents;
} env_t_198;
env_t_198 make_env_t_198(array_array_array_number_t relatives,array_number_t parents) {
	env_t_198 env;
...
value_t lambda198(env_t_198* env191, array_array_array_number_t acc, index_t i) {
	array_array_array_number_t relatives190 = env191->relatives;
	array_number_t parents189 = env191->parents;
	array_array_array_number_t ite197 = 0;
	if((parents189->arr[i]) == (-1)) {
	...
	newMatrix->arr[0] = relatives190->arr[i];;
		ite197 = TOP_LEVEL_linalg_matrix3DConcat(acc, newMatrix);;
	} else {
		array_array_array_number_t newMatrix = (array_array_array_number_t)storage_alloc(sizeof(int) * 2);
	newMatrix->length=1;
	newMatrix->arr = (array_array_number_t*)storage_alloc(sizeof(array_array_number_t) * 1);
	newMatrix->arr[0] = TOP_LEVEL_linalg_matrixMult(acc->arr[(int)(parents189->arr[i])], relatives190->arr[i]);;
		ite197 = TOP_LEVEL_linalg_matrix3DConcat(acc, newMatrix);;
	}
	value_t res;
	res.array_array_array_number_t_value = ite197;
	return res;
}
array_array_array_number_t TOP_LEVEL_usecases_ht_relatives_to_absolutes(array_array_array_number_t relatives, array_number_t parents) {
	array_number_t array196 = (array_number_t)storage_alloc(sizeof(int) * 2);
	array196->length=0;
	array196->arr = (number_t*)storage_alloc(sizeof(number_t) * 0);
	;
	array_array_number_t array195 = (array_array_number_t)storage_alloc(sizeof(int) * 2);
	array195->length=1;
	array195->arr = (array_number_t*)storage_alloc(sizeof(array_number_t) * 1);
	array195->arr[0] = array196;;
	...
	init->arr[0] = array195;;
	env_t_198 env_t_198_value = make_env_t_198(relatives,parents); closure_t closure193 = make_closure(lambda198, &env_t_198_value);
	card_t macroDef194 = relatives->length;
	return TOP_LEVEL_linalg_iterateMatrix3D(closure193, init, 0, (macroDef194) - (1));
}
typedef struct env_t_209 {
	array_number_t scale;
} env_t_209;
env_t_209 make_env_t_209(array_number_t scale) {
	env_t_209 env;
	env.scale = scale;
	return env;
}

value_t lambda209(env_t_209* env200, array_number_t row) {
	array_number_t scale199 = env200->scale;
	value_t res;
	res.array_number_t_value = TOP_LEVEL_linalg_mult_vec_elementwise(row, scale199);
	return res;
}
typedef empty_env_t env_t_210;


value_t lambda210(env_t_210* env203, number_t x) {
...
array_array_number_t TOP_LEVEL_usecases_ht_apply_global_transform(array_array_number_t pose_params, array_array_number_t positions) {
	...
	env_t_209 env_t_209_value = make_env_t_209(scale); closure_t closure202 = make_closure(lambda209, &env_t_209_value);
	array_array_number_t R1 = TOP_LEVEL_linalg_matrixMap(closure202, R);
	array_array_number_t array207 = (array_array_number_t)storage_alloc(sizeof(int) * 2);
	array207->length=1;
	array207->arr = (array_number_t*)storage_alloc(sizeof(array_number_t) * 1);
	array207->arr[0] = pose_params->arr[2];;
	array_array_number_t T = TOP_LEVEL_linalg_matrixConcatCol(R1, TOP_LEVEL_linalg_matrixTranspose(array207));
	env_t_210 env_t_210_value = make_empty_env(); closure_t closure205 = make_closure(lambda210, &env_t_210_value);
	card_t macroDef206 = positions->arr[0]->length;
	array_number_t ones = TOP_LEVEL_linalg_vectorMap(closure205, TOP_LEVEL_linalg_vectorRange(1, macroDef206));
	array_array_number_t array208 = (array_array_number_t)storage_alloc(sizeof(int) * 2);
	array208->length=1;
	array208->arr = (array_number_t*)storage_alloc(sizeof(array_number_t) * 1);
	array208->arr[0] = ones;;
	array_array_number_t positions_homog = TOP_LEVEL_linalg_matrixConcat(positions, array208);
	return TOP_LEVEL_linalg_matrixMult(T, positions_homog);
}
typedef empty_env_t env_t_226;


value_t lambda226(env_t_226* env211, array_array_number_t m1, array_array_number_t m2) {
...
env_t_227 make_env_t_227(array_array_number_t weights,array_array_array_number_t transforms,array_array_number_t base_positions) {
	env_t_227 env;
	env.weights = weights;
...
value_t lambda227(env_t_227* env219, array_array_number_t acc, index_t i_transform) {
	array_array_number_t weights218 = env219->weights;
	array_array_array_number_t transforms217 = env219->transforms;
	array_array_number_t base_positions216 = env219->base_positions;
	array_array_number_t curr_positions = TOP_LEVEL_linalg_matrixMult(matrix_slice(transforms217->arr[i_transform], 0, 2), base_positions216);
	card_t macroDef215 = base_positions216->length;
	array_array_number_t w = TOP_LEVEL_linalg_matrixFillFromVector(macroDef215, weights218->arr[i_transform]);
	value_t res;
...
array_array_number_t TOP_LEVEL_usecases_ht_get_skinned_vertex_positions(index_t is_mirrored, card_t n_bones, array_array_number_t pose_params, array_array_array_number_t base_relatives, array_number_t parents, array_array_array_number_t inverse_base_absolutes, array_array_number_t base_positions, array_array_number_t weights) {
	...
	env_t_226 env_t_226_value = make_empty_env(); closure_t closure213 = make_closure(lambda226, &env_t_226_value);
	array_array_array_number_t transforms = TOP_LEVEL_linalg_matrix3DMap2(closure213, absolutes, inverse_base_absolutes);
	card_t macroDef214 = base_positions->arr[0]->length;
	card_t n_verts = macroDef214;
	array_array_number_t init_positions = TOP_LEVEL_linalg_matrixFill(3, n_verts, 0);
	env_t_227 env_t_227_value = make_env_t_227(weights,transforms,base_positions); closure_t closure221 = make_closure(lambda227, &env_t_227_value);
	card_t macroDef222 = transforms->length;
	array_array_number_t positions = TOP_LEVEL_linalg_iterateMatrix(closure221, init_positions, 0, (macroDef222) - (1));
	...
	array223->arr[1] = 0;
	array223->arr[2] = 0;;
		array_number_t array224 = (array_number_t)storage_alloc(sizeof(int) * 2);
	array224->length=3;
	array224->arr = (number_t*)storage_alloc(sizeof(number_t) * 3);
	array224->arr[0] = 0;
	array224->arr[1] = 1;
	array224->arr[2] = 0;;
	...
	mirror_matrix->arr[0] = array223;
	mirror_matrix->arr[1] = array224;
	mirror_matrix->arr[2] = array225;;
...
env_t_235 make_env_t_235(array_array_number_t vertex_positions,array_array_number_t points,index_t n_corr,array_number_t correspondences) {
	env_t_235 env;
...
value_t lambda235(env_t_235* env232, number_t i) {
	array_array_number_t vertex_positions231 = env232->vertex_positions;
	array_array_number_t points230 = env232->points;
	index_t n_corr229 = env232->n_corr;
	array_number_t correspondences228 = env232->correspondences;
	index_t ind = (int)(i);
	index_t r = (ind) / (n_corr229);
	index_t c = (ind) % (n_corr229);
	value_t res;
	res.number_t_value = (points230->arr[r]->arr[c]) - (vertex_positions231->arr[r]->arr[(int)(correspondences228->arr[c])]);
	return res;
}
array_number_t TOP_LEVEL_usecases_ht_hand_objective(index_t is_mirrored, array_number_t param, array_number_t correspondences, array_array_number_t points, card_t n_bones, array_array_array_number_t base_relatives, array_number_t parents, array_array_array_number_t inverse_base_absolutes, array_array_number_t base_positions, array_array_number_t weights) {
	...
	index_t dims = 3;
	env_t_235 env_t_235_value = make_env_t_235(vertex_positions,points,n_corr,correspondences); closure_t closure234 = make_closure(lambda235, &env_t_235_value);
	array_number_t err = TOP_LEVEL_linalg_vectorMap(closure234, TOP_LEVEL_linalg_vectorRange(0, ((dims) * (n_corr)) - (1)));
	return err;
}
typedef empty_env_t env_t_246;


value_t lambda246(env_t_246* env236, number_t r) {
	
	value_t res;
	res.array_number_t_value = TOP_LEVEL_linalg_vectorRange(((int)(r)) * (4), (((int)(r)) * (4)) + (3));
	return res;
}
void TOP_LEVEL_usecases_ht_test_ht(void unitVar0) {
	...
	array240->arr[1] = 2;
	array240->arr[2] = 3;;
	array_number_t array241 = (array_number_t)storage_alloc(sizeof(int) * 2);
	array241->length=3;
	array241->arr = (number_t*)storage_alloc(sizeof(number_t) * 3);
	array241->arr[0] = 4;
	array241->arr[1] = 5;
	array241->arr[2] = 6;;
	...
	mat1->arr[1] = array241;
	mat1->arr[2] = array242;;
	env_t_246 env_t_246_value = make_empty_env(); closure_t closure238 = make_closure(lambda246, &env_t_246_value);
	array_array_number_t base_rel = TOP_LEVEL_linalg_vectorMapToMatrix(closure238, TOP_LEVEL_linalg_vectorRange(1, 4));
