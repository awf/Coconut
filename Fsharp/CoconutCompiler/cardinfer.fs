...
let rec inferCardinality (exp: Expr): Expr =
  ...
  | Patterns.NewArray(tp, es)       -> 
    ...
    <@@ vectorShape (%%ce1: Shape) (%%N: Cardinality) @@>
  | DerivedPatterns.SpecificCall <@ corelang.vectorBuild @> (_, _, [e0; e1]) ->
    let ce0 = C e0
    let ce1 = C e1
    let zeroCard = Expr.Value(Card 0, typeof<Cardinality>)
    <@@ vectorShape (flatShape ((%%ce1: Cardinality -> Cardinality) %%zeroCard)) (%%ce0: Cardinality) @@>
  | ArrayLength(e0) ->
    let ce0 = C e0
    <@@ shapeCard %%ce0 @@>
  | ArrayGet(e0, e1) ->
